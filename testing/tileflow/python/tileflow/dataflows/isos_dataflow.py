...
def conv3x3_conv3x3_dataflow_2levels(ctx, tI, tW1, tW2, batch, height, width, in_channel, out_channel_1, out_channel_2, define_tiling_space=True):
    ...
    with ctx.tile("L2", [b, w2], "Temporal"):
        with ctx.pipeline():
            with ctx.tile("L2", [w1], "Spatial"):
                with ctx.tile("L1", [h, w0, c1, l1], "Temporal"):
                    with ctx.tile("L1", [c0, l0], "Spatial"):
                        with ctx.tile("L0", [r, s], "Temporal"):
                            tA[b, h, w, l] = tA[b, h, w, l] + \
                                tI[b, h + r, w + s, c] * tW1[l, r, s, c]
                with ctx.tile("L1", [h, w0, l1, k1], "Temporal"):
...
def conv3x3_conv3x3_dataflow_3levels(ctx, tI, tW1, tW2, batch, height, width, in_channel, out_channel_1, out_channel_2, define_tiling_space=True):
    ...
    with ctx.tile("L3", [b, w4], "Temporal"):
        with ctx.tile("L3", [w3], "Spatial"):
            with ctx.tile("L2", [w2], "Temporal"):
                with ctx.pipeline():
                    with ctx.tile("L2", [w1], "Spatial"):
                        with ctx.tile("L1", [h, w0, c1, l1], "Temporal"):
                            with ctx.tile("L1", [c0, l0], "Spatial"):
...
def get_isos_dataflow(levels, batch, height, width, in_channel, out_channel_1, out_channel_2, define_tiling_space=True):
