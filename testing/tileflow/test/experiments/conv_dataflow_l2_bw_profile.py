...
def replay(dataflow_name, l1_bw, l2_bw, logfile, hw_id, batch, height, width, in_channel, out_channel_1, out_channel_2, metric_type, debug=False, resource_check=True, define_tiling_space=True, layout="nhwc", second_kernel_size=3):
    hw = [(2, get_edge_small(L1_BW=l1_bw, L2_BW=l2_bw)), (3, get_cloud_small(L2_BW=l2_bw))]
    levels = hw[hw_id][0]
...
def replay_config(dataflow_name, l1_bw, l2_bw, config, hw_id, batch, height, width, in_channel, out_channel_1, out_channel_2, metric_type, debug=False, resource_check=True, define_tiling_space=True, layout="nhwc", second_kernel_size=3):
    hw = [(2, get_edge_small(L1_BW=l1_bw, L2_BW=l2_bw)), (3, get_cloud_small(L2_BW=l2_bw))]
    levels = hw[hw_id][0]
    hw_config = acc.tileflow_accelerator_generator(
        hw[hw_id][1])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--l2_bw", type=int,
                        help="The L2 BW lower bound", default=100)
    ...
    if args.inference:
        ...
        results = []
        if args.config_key:
            for l2_bw in range(args.l2_bw, 25, 1):
                perf = replay_config(args.dataflow, args.l1_bw, l2_bw, args.config_key.replace("'", '"'), args.inference_hw, batch, height, width, in_channel, out_channel_1, out_channel_2,
                ...
                results.append((l2_bw, perf))
        else:
            for l2_bw in range(args.l2_bw, 25, 1):
                perf = replay(args.dataflow, args.l1_bw, l2_bw, args.logfile, args.inference_hw, batch, height, width, in_channel, out_channel_1, out_channel_2,
                              args.metric, args.debug, args.check_resource, args.define_tiling_space, args.layout, second_kernel_size=args.second_kernel_size)
                perf = perf[args.metric]
                results.append((l2_bw, perf))

        with open("l2_bw_result.csv", "w") as fout:
            fout.write(f"L2_BW,{args.metric}\n")
            for res in results:
        ...
        for shape in shapes[args.begin:args.begin+args.number]:
            ...
            for l2_bw in range(args.l2_bw, 25, 1):
                ...
                hw_config = acc.tileflow_accelerator_generator(
                    hw_func(L1_BW=args.l1_bw, L2_BW=l2_bw))
                perf, key, config = run(
