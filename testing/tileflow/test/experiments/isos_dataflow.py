...
def run(levels, hw_config, batch, height, width, in_channel, out_channel_1, out_channel_2, trials, metric_type, debug=False, resource_check=True, define_tiling_space=False, layout="nhwc", second_kernel_size=3):
    dataflow = td.get_isos_dataflow(
...
"""example
python isos_dataflow.py --metric=1e9/latency --trials 1 |& tee trace.log
"""
    ...
    if args.inference:
        ...
        for shape in shapes[args.begin:args.begin+args.number]:
            ...
            for i, (levels, hw) in enumerate(zip([2, 3], [get_edge_small(), get_cloud_small()])):
                ...
                perf, key, config = run(
                    levels, hw_config, batch, height, width, in_channel, out_channel_1, out_channel_2, trials, metric_type=metric_type, debug=args.debug, resource_check=args.check_resource, define_tiling_space=args.define_tiling_space, layout=args.layout, second_kernel_size=args.second_kernel_size)
