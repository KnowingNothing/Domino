...
def div4d_compute(ctx, tQ, tK, batch, num_heads, seq_len, hidden, levels):
    """
    tQ: [batch, num_heads, seq_len, seq_len]
    tK: [batch, num_heads, seq_len]
    """
    ...
    tC = ops.div4D(ctx, tQ, tK, loop_b, loop_h, loop_m,
...
def run(levels, hw_config, batch, num_heads, seq_len, hidden, trials):
    ...
    def static_div4d(ctx, B, H, M, N):
        ...
        with dir.NameScope(only_capital=True):
            ...
            tK = dir.Tensor([B, H, M], name="K", dtype="int16", ctx=ctx)
            [tF], loops = div4d_compute(
                ctx, tQ, tK, *[B, H, M, N], levels=levels)
            return [tQ, tK], [tF], loops

    sp_beg = time.time()
    space = get_space(static_div4d, [
    ...
    for ep in tqdm(range(epoch)):
        ...
        for i in range(steps):
            ...
            candidate = generate_candidate(
                space, static_div4d, [batch, num_heads, seq_len, hidden])
