...
def sum4d_compute(ctx, tQ, batch, num_heads, seq_len, hidden, levels):
    """
    ...
    tC = ops.sum4D(ctx, tQ, loop_b, loop_h, loop_m,
...
def run(levels, hw_config, batch, num_heads, seq_len, hidden, trials):
    ...
    def static_sum4d(ctx, B, H, M, N):
        ...
        with dir.NameScope(only_capital=True):
            ...
            [tF], loops = sum4d_compute(
                ctx, tQ, *[B, H, M, N], levels=levels)
            return [tQ], [tF], loops

    sp_beg = time.time()
    space = get_space(static_sum4d, [
    ...
    for ep in tqdm(range(epoch)):
        ...
        for i in range(steps):
            ...
            candidate = generate_candidate(
                space, static_sum4d, [batch, num_heads, seq_len, hidden])
