...
def run(levels, hw_config, batch, num_heads, seq_len, hidden):

    def get_workload(ctx, B, H, M, N):
    ...
    ctx = Context()
    inputs, outputs, loops = get_workload(ctx, batch, num_heads, seq_len, hidden)
...
if __name__ == "__main__":
    ...
    hw = get_edge_small()
    hw_config = acc.tileflow_accelerator_generator(hw)
    workload, kernel = run(levels, hw_config, batch, num_heads, seq_len, hidden)
